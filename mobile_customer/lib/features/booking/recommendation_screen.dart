        ...
        data: (payload) {
          ...
          for (final id in productIds) {
            ...
            if (product.variants.isEmpty) {
              ...
              final sel = productSelections[id]?.variantSelections ?? {};
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            Checkbox(
              value: isComplete,
              onChanged: (_) {}, // disabled; uses onTap of row
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)),
            ),
            const SizedBox(width: 12),
            Expanded(
...
class _ProductVariantSelectorSheetState extends ConsumerState<_ProductVariantSelectorSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final productSelections = ref.watch(productSelectionProvider).state;
    final selections = productSelections[productId]?.variantSelections ?? {};
    final variants = product.variants;
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, i) {
              ...
              return RadioListTile<String>(
                ...
                onChanged: (val) {
                  if (val != null) {
                    ref.read(productSelectionProvider.notifier).selectOption(product.productId, variant.variantId, val);
                    // Also update the selectedAccessoriesProvider to reflect completion status
                    final updatedSelections = ref.read(productSelectionProvider).state[product.productId]?.variantSelections ?? {};
                    final isNowComplete = _isProductComplete(product, updatedSelections);
                    ref.read(selectedAccessoriesProvider.notifier).setSelected(product.productId, isNowComplete);
                  }
                },
              );
            },
      ...
      child: Column(
        ...
        children: [
          ...
          FilledButton(
            onPressed: () {
              // Ensure selection is marked (should already be true)
              ref.read(selectedAccessoriesProvider.notifier).setSelected(product.productId, true);
              Navigator.pop(context);
            },
            style: FilledButton.styleFrom(
              padding: const EdgeInsets.symmetric(vertical: 16, horizontal: 32),
